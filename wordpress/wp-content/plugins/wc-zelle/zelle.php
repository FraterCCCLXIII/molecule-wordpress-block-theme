 ...
 */
function wc_zelle_plugin_deactivate() {
	if ( ! defined( 'WCZELLE_PLUGIN_DIR' ) ) {
		return;
	}
	$path = WCZELLE_PLUGIN_DIR . 'includes/class-wc-zelle-unpaid-cancellation.php';
	if ( is_readable( $path ) ) {
		require_once $path;
		if ( class_exists( 'WC_Zelle_Unpaid_Cancellation' ) ) {
			WC_Zelle_Unpaid_Cancellation::clear_scheduled_hook();
		}
	}
}

register_deactivation_hook( __FILE__, 'wc_zelle_plugin_deactivate' );
...
if ( function_exists( 'zelle_fs' ) ) {
    ...
    add_filter( 'woocommerce_email_classes', 'wc_zelle_register_payment_received_email_class', 20, 1 );
    /**
     * Register customer email (after WC_Email base is loaded by WC_Emails::init).
     *
     * @param array<string, WC_Email> $emails Registered emails.
     * @return array<string, WC_Email>
     */
    function wc_zelle_register_payment_received_email_class( $emails ) {
        if ( ! defined( 'WCZELLE_PLUGIN_DIR' ) ) {
            return $emails;
        }
        if ( ! class_exists( 'WC_Email_Zelle_Payment_Received', false ) ) {
            require_once WCZELLE_PLUGIN_DIR . 'includes/emails/class-wc-email-zelle-payment-received.php';
        }
        $emails['WC_Email_Zelle_Payment_Received'] = new WC_Email_Zelle_Payment_Received();
        return $emails;
    }
    ...
    function zelle_init_gateway_class() {
        ...
        if ( class_exists( 'WC_Payment_Gateway' ) ) {
            ...
            require_once WCZELLE_PLUGIN_DIR . 'includes/class-wc-zelle-unpaid-cancellation.php';
            WC_Zelle_Unpaid_Cancellation::init();
        } else {
            require_once WCZELLE_PLUGIN_DIR . 'includes/notifications/woocommerce.php';
        }
    }
